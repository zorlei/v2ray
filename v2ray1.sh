...
backup="/etc/v2ray/zorlei_v2ray_backup.conf"
...
_load() {
	local _dir="/etc/v2ray/zorlei/v2ray/src/"
	. "${_dir}$@"
}
_sys_timezone() {
	IS_OPENVZ=
...
path_config() {
	echo
	while :; do
		echo -e "请输入想要 ${magenta}用来分流的路径$none , 例如 /zorlei , 那么只需要输入 zorlei 即可"
		read -p "$(echo -e "(默认: [${cyan}zorlei$none]):")" path
		[[ -z $path ]] && path="zorlei"
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}zorlei.com$none)"): " sspass
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			...
			echo
			echo -e " 请确保你有完整的上传 zorlei.com 的 V2Ray 一键安装脚本 & 管理脚本到当前 ${green}$(pwd) $none目录下"
			echo
			exit 1
		fi
		...
		pushd /tmp
		git clone https://github.com/zorlei/v2ray -b "$_gitbranch" /etc/v2ray/zorlei/v2ray --depth=1
		popd

	fi

	if [[ ! -d /etc/v2ray/zorlei/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/zorlei/v2ray/config/backup.conf $backup
	cp -f /etc/v2ray/zorlei/v2ray/v2ray.sh $_v2ray_sh
	chmod +x $_v2ray_sh
...
backup_config() {
	...
	if [[ $shadowsocks ]]; then
		sed -i "42s/=/=true/; 45s/=6666/=$ssport/; 48s/=zorlei.com/=$sspass/; 51s/=chacha20-ietf/=$ssciphers/" $backup
	fi
	[[ $v2ray_transport == [45] ]] && sed -i "36s/=zorlei.com/=$domain/" $backup
	[[ $caddy ]] && sed -i "39s/=/=true/" $backup
	[[ $ban_ad ]] && sed -i "54s/=/=true/" $backup
	if [[ $is_path ]]; then
		sed -i "57s/=/=true/; 60s/=zorlei/=$path/" $backup
...
install() {
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/zorlei/v2ray ]]; then
		echo
		...
		echo
		exit 1
	elif [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f /etc/v2ray/zorlei_v2ray_backup.txt && -d /etc/v2ray/zorlei/v2ray ]]; then
		echo
...
while :; do
	...
	echo
	echo "帮助说明: https://zorlei.com/post/1/"
	echo
	echo "搭建教程: https://zorlei.com/post/2/"
	echo
	echo " 1. 安装"
	echo
	echo " 2. 卸载"
